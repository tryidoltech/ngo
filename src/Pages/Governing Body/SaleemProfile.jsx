...
import { SaleemAhmedQureshi } from "../../../data/Data"; // Adjust the path to your data file

const SaleemProfile = () => {
  // Destructuring data from the imported `SaleemAhmedQureshi` object
  const { name, title, contact, bio, otherPosts } = SaleemAhmedQureshi;
        ...
        <div className="w-full px-24 max-md:px-10 py-20 max-md:flex-col flex">
          <div className="min-w-[18vw] max-md:w-full">
            <img
              ...
              alt={`${name}'s Profile`}
            />
            <h1 className="text-xl font-semibold py-2">{SaleemAhmedQureshi.name}</h1>
            <h5 className="italic text-gray-600">{SaleemAhmedQureshi.title}</h5>
